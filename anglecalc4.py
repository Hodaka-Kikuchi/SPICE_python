...
def angle_calc4(dmono,dana,astar, bstar, cstar, U,B,UB, bpe, bpc2, bpmu, bpnu, bp, fixe, w_config, hw_ini, hw_fin, h_cal_ini,h_cal_fin,k_cal_ini,k_cal_fin, l_cal_ini,l_cal_fin, points):
    ...
    # 変数はhkl
    # --- 各軸を統一的に生成 ---
    def generate_axis(ini, fin, points):
        if ini != fin:
            return np.linspace(ini, fin, points)
        else:
            return np.full(points, ini)

    h_tab = generate_axis(h_cal_ini, h_cal_fin, points)
    k_tab = generate_axis(k_cal_ini, k_cal_fin, points)
    l_tab = generate_axis(l_cal_ini, l_cal_fin, points)
    hw_tab = generate_axis(hw_ini, hw_fin, points)

    if fixe == 0:  # Ei fix
        Ei = np.full(points, bpe)
        Ef = bpe - hw_tab
    elif fixe == 1:  # Ef fix
        Ei = bpe + hw_tab
        Ef = np.full(points, bpe)
    ...
    for i in range(len(h_tab)):
        ...
        ki_cal=(Ei[i]/2.072)**(1/2)
        kf_cal=(Ef[i]/2.072)**(1/2)
        ...
        # C1とA1の計算
        C1 = np.degrees(np.arcsin((2 * np.pi / d_mono) / (2 * np.sqrt(Ei[i] / 2.072))))
        A1 = 2 * C1
        
        # C3とA3の計算
        C3 = np.degrees(np.arcsin((2 * np.pi / d_ana) / (2 * np.sqrt(Ef[i] / 2.072))))
        A3 = 2 * C3
